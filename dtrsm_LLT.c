...
#define PARSEC_dtrsm_LLT_NB_TASK_CLASSES 4
#define PARSEC_dtrsm_LLT_NB_DATA 2

typedef struct __parsec_dtrsm_LLT_internal_taskpool_s __parsec_dtrsm_LLT_internal_taskpool_t;
struct parsec_dtrsm_LLT_internal_taskpool_s;

/** Predeclarations of the parsec_task_class_t */
static const parsec_task_class_t dtrsm_LLT_dgemm;
static const parsec_task_class_t dtrsm_LLT_write_B0;
static const parsec_task_class_t dtrsm_LLT_read_B0;
static const parsec_task_class_t dtrsm_LLT_dtrsm;
/** Predeclarations of the parameters */
static const parsec_flow_t flow_of_dtrsm_LLT_dgemm_for_E;
static const parsec_flow_t flow_of_dtrsm_LLT_dgemm_for_C;
static const parsec_flow_t flow_of_dtrsm_LLT_dgemm_for_D;
static const parsec_flow_t flow_of_dtrsm_LLT_write_B0_for_B;
static const parsec_flow_t flow_of_dtrsm_LLT_read_B0_for_B;
static const parsec_flow_t flow_of_dtrsm_LLT_dtrsm_for_B;
static const parsec_flow_t flow_of_dtrsm_LLT_dtrsm_for_A;
#line 2 "dtrsm_LLT.jdf"
/*
...
struct __parsec_dtrsm_LLT_internal_taskpool_s {
    parsec_dtrsm_LLT_taskpool_t super;
...
static int dtrsm_LLT_profiling_array[4 * PARSEC_dtrsm_LLT_NB_TASK_CLASSES] = { -1 };	/* 2 pairs (begin, end) per task, times two because each task class has an internal_init task */
#else /* defined(PARSEC_PROF_TRACE_PTG_INTERNAL_INIT) */
static int dtrsm_LLT_profiling_array[2 * PARSEC_dtrsm_LLT_NB_TASK_CLASSES] = { -1 };	/* 2 pairs (begin, end) per task */
...
static inline int32_t dtrsm_LLT_dgemm_inline_c_expr1_line_92(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							     const __parsec_dtrsm_LLT_dgemm_parsec_assignment_t *
							     assignments)
{
    (void) __parsec_tp;
    ...
    return FLOPS_DGEMM(descB->mb, CLEAN_NB(descB, n), CLEAN_MB(descB, k));
#line 108 "dtrsm_LLT.c"
}

static inline int32_t dtrsm_LLT_dtrsm_inline_c_expr2_line_26(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							     const __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t *
							     assignments)
{
    (void) __parsec_tp;
...
#define dgemm_pred(k, m, n) (((parsec_data_collection_t*)(__parsec_tp->super._g_descA))->myrank == rank_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - m)))
#define write_B0_pred(k) (((parsec_data_collection_t*)(__parsec_tp->super._g_descB))->myrank == rank_of_descB(((descB->mt - 1) - k), 0))
#define read_B0_pred(k) (((parsec_data_collection_t*)(__parsec_tp->super._g_descB))->myrank == rank_of_descB(((descB->mt - 1) - k), 0))
#define dtrsm_pred(k, n) (((parsec_data_collection_t*)(__parsec_tp->super._g_descA))->myrank == rank_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - k)))
...
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp = (const __parsec_dtrsm_LLT_internal_taskpool_t *) tp;
    __parsec_dtrsm_LLT_dgemm_parsec_assignment_t ascopy, *assignment = &ascopy;
    uintptr_t __parsec_id = 0;
    memcpy(assignment, as, sizeof(__parsec_dtrsm_LLT_dgemm_parsec_assignment_t));
					     ...
					     void *user_data)
{
    ...
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp = (__parsec_dtrsm_LLT_internal_taskpool_t *) user_data;
...
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp = (const __parsec_dtrsm_LLT_internal_taskpool_t *) tp;
    __parsec_dtrsm_LLT_write_B0_parsec_assignment_t ascopy, *assignment = &ascopy;
    uintptr_t __parsec_id = 0;
    memcpy(assignment, as, sizeof(__parsec_dtrsm_LLT_write_B0_parsec_assignment_t));
    const int k = assignment->k.value;
    int __jdf2c_k_min = 0;
    __parsec_id += (k - __jdf2c_k_min);
    (void) __parsec_tp;
...
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp = (const __parsec_dtrsm_LLT_internal_taskpool_t *) tp;
    __parsec_dtrsm_LLT_read_B0_parsec_assignment_t ascopy, *assignment = &ascopy;
    uintptr_t __parsec_id = 0;
    memcpy(assignment, as, sizeof(__parsec_dtrsm_LLT_read_B0_parsec_assignment_t));
    const int k = assignment->k.value;
    int __jdf2c_k_min = 0;
    __parsec_id += (k - __jdf2c_k_min);
    (void) __parsec_tp;
...
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp = (const __parsec_dtrsm_LLT_internal_taskpool_t *) tp;
    __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t ascopy, *assignment = &ascopy;
    uintptr_t __parsec_id = 0;
    memcpy(assignment, as, sizeof(__parsec_dtrsm_LLT_dtrsm_parsec_assignment_t));
    const int k = assignment->k.value;
    int __jdf2c_k_min = 0;
    const int n = assignment->n.value;
    int __jdf2c_n_min = 0;
...
static inline int32_t minexpr_of_symb_dtrsm_LLT_dgemm_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							    const __parsec_dtrsm_LLT_dgemm_parsec_assignment_t * locals)
{
    (void) __parsec_tp;
    (void) locals;
    return 0;
...
static const parsec_expr_t minexpr_of_symb_dtrsm_LLT_dgemm_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) minexpr_of_symb_dtrsm_LLT_dgemm_k_fct}
		      }
};

static inline int maxexpr_of_symb_dtrsm_LLT_dgemm_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							const __parsec_dtrsm_LLT_dgemm_parsec_assignment_t * locals)
{
...
static const parsec_expr_t maxexpr_of_symb_dtrsm_LLT_dgemm_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) maxexpr_of_symb_dtrsm_LLT_dgemm_k_fct}
		      }
};
static const parsec_symbol_t symb_dtrsm_LLT_dgemm_k = {.name = "k",.context_index = 0,.min =
	&minexpr_of_symb_dtrsm_LLT_dgemm_k,.max = &maxexpr_of_symb_dtrsm_LLT_dgemm_k,.cst_inc = 1,.expr_inc =
	NULL,.flags = 0x0 };

static inline int minexpr_of_symb_dtrsm_LLT_dgemm_m_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							const __parsec_dtrsm_LLT_dgemm_parsec_assignment_t * locals)
{
    const int k = locals->k.value;
    const int m = locals->m.value;
...
static const parsec_expr_t minexpr_of_symb_dtrsm_LLT_dgemm_m = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) minexpr_of_symb_dtrsm_LLT_dgemm_m_fct}
		      }
};

static inline int maxexpr_of_symb_dtrsm_LLT_dgemm_m_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							const __parsec_dtrsm_LLT_dgemm_parsec_assignment_t * locals)
{
    const int k = locals->k.value;
    (void) k;
...
static const parsec_expr_t maxexpr_of_symb_dtrsm_LLT_dgemm_m = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) maxexpr_of_symb_dtrsm_LLT_dgemm_m_fct}
		      }
};
static const parsec_symbol_t symb_dtrsm_LLT_dgemm_m = {.name = "m",.context_index = 1,.min =
	&minexpr_of_symb_dtrsm_LLT_dgemm_m,.max = &maxexpr_of_symb_dtrsm_LLT_dgemm_m,.cst_inc = 1,.expr_inc =
	NULL,.flags = 0x0 };

static inline int32_t expr_of_symb_dtrsm_LLT_dgemm_n_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							 const __parsec_dtrsm_LLT_dgemm_parsec_assignment_t * locals)
{
    (void) __parsec_tp;
    (void) locals;
...
static const parsec_expr_t expr_of_symb_dtrsm_LLT_dgemm_n = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) expr_of_symb_dtrsm_LLT_dgemm_n_fct}
		      }
};
static const parsec_symbol_t symb_dtrsm_LLT_dgemm_n = {.name = "n",.context_index = 2,.min =
	&expr_of_symb_dtrsm_LLT_dgemm_n,.max = &expr_of_symb_dtrsm_LLT_dgemm_n,.cst_inc = 0,.expr_inc = NULL,.flags =
	PARSEC_SYMBOL_IS_STANDALONE };

static inline int affinity_of_dtrsm_LLT_dgemm(__parsec_dtrsm_LLT_dgemm_task_t * this_task, parsec_data_ref_t * ref)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    ...
    /* Compute data key */
    ref->key = ref->dc->data_key(ref->dc, ((descB->mt - 1) - k), ((descB->mt - 1) - m));
    return 1;
}

static inline int32_t property_2_flops_of_dtrsm_LLT_dgemm_as_expr_fct(const __parsec_dtrsm_LLT_internal_taskpool_t *
								      __parsec_tp,
								      const __parsec_dtrsm_LLT_dgemm_parsec_assignment_t
								      * locals)
{
    (void) __parsec_tp;
    (void) locals;
    return dtrsm_LLT_dgemm_inline_c_expr1_line_92(__parsec_tp, locals);
}

static const parsec_expr_t property_2_flops_of_dtrsm_LLT_dgemm_as_expr = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) property_2_flops_of_dtrsm_LLT_dgemm_as_expr_fct}
		      }
};

static const parsec_property_t properties_of_dtrsm_LLT_dgemm[2] = {
    {.name = "flops",.expr = &property_2_flops_of_dtrsm_LLT_dgemm_as_expr},
    {.name = NULL,.expr = NULL}
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep1_atline_103_fct(const
										     __parsec_dtrsm_LLT_internal_taskpool_t
										     * __parsec_tp,
										     const
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep1_atline_103 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep1_atline_103_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_dgemm_for_E_dep1_atline_103 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep1_atline_103,	/* (k >= 1) */
    .ctl_gather_nb = NULL,
    .task_class_id = 3,		/* dtrsm_LLT_dgemm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_dgemm_for_E,
    .dep_index = 2,
    .dep_datatype_index = 2,
    .belongs_to = &flow_of_dtrsm_LLT_dgemm_for_E,
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep2_atline_104_fct(const
										     __parsec_dtrsm_LLT_internal_taskpool_t
										     * __parsec_tp,
										     const
										     __parsec_dtrsm_LLT_dgemm_parsec_assignment_t
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep2_atline_104 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep2_atline_104_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_dgemm_for_E_dep2_atline_104 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep2_atline_104,	/* (0 == k) */
    .ctl_gather_nb = NULL,
    .task_class_id = 1,		/* dtrsm_LLT_read_B0 */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_read_B0_for_B,
    .dep_index = 3,
    .dep_datatype_index = 2,
    .belongs_to = &flow_of_dtrsm_LLT_dgemm_for_E,
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep3_atline_105_fct(const
										     __parsec_dtrsm_LLT_internal_taskpool_t
										     * __parsec_tp,
										     const
										     __parsec_dtrsm_LLT_dgemm_parsec_assignment_t
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep3_atline_105 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep3_atline_105_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_dgemm_for_E_dep3_atline_105 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep3_atline_105,	/* ((1 + k) == m) */
    .ctl_gather_nb = NULL,
    .task_class_id = 0,		/* dtrsm_LLT_dtrsm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_dtrsm_for_B,
    .dep_index = 0,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_dgemm_for_E,
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep4_atline_106_fct(const
										     __parsec_dtrsm_LLT_internal_taskpool_t
										     * __parsec_tp,
										     const
										     __parsec_dtrsm_LLT_dgemm_parsec_assignment_t
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep4_atline_106 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep4_atline_106_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_dgemm_for_E_dep4_atline_106 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_dgemm_for_E_dep4_atline_106,	/* (m >= (k + 2)) */
    .ctl_gather_nb = NULL,
    .task_class_id = 3,		/* dtrsm_LLT_dgemm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_dgemm_for_E,
...
static const parsec_flow_t flow_of_dtrsm_LLT_dgemm_for_E = {
    ...
    .dep_in = {&flow_of_dtrsm_LLT_dgemm_for_E_dep1_atline_103,
	       &flow_of_dtrsm_LLT_dgemm_for_E_dep2_atline_104},
    .dep_out = {&flow_of_dtrsm_LLT_dgemm_for_E_dep3_atline_105,
		&flow_of_dtrsm_LLT_dgemm_for_E_dep4_atline_106}
};

static parsec_data_t *flow_of_dtrsm_LLT_dgemm_for_C_dep1_atline_100_direct_access(const
										  __parsec_dtrsm_LLT_internal_taskpool_t
										  * __parsec_tp,
										  const
										  __parsec_dtrsm_LLT_dgemm_parsec_assignment_t
										  * assignments)
{
    ...
    (void) m;
    (void) n;
    if (__parsec_tp->super.super.context->my_rank ==
	(int32_t) rank_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - m)))
	return data_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - m));
    return NULL;
}

static const parsec_dep_t flow_of_dtrsm_LLT_dgemm_for_C_dep1_atline_100 = {
    .cond = NULL,		/*  */
    .ctl_gather_nb = NULL,
    .task_class_id = PARSEC_LOCAL_DATA_TASK_CLASS_ID,	/* dtrsm_LLT_descA */
    .direct_data = (parsec_data_lookup_func_t) & flow_of_dtrsm_LLT_dgemm_for_C_dep1_atline_100_direct_access,
    .dep_index = 0,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_dgemm_for_C,
};
...
static const parsec_flow_t flow_of_dtrsm_LLT_dgemm_for_C = {
    ...
    .dep_in = {&flow_of_dtrsm_LLT_dgemm_for_C_dep1_atline_100},
    .dep_out = {NULL}
};

static const parsec_dep_t flow_of_dtrsm_LLT_dgemm_for_D_dep1_atline_102 = {
    .cond = NULL,		/*  */
    .ctl_gather_nb = NULL,
    .task_class_id = 0,		/* dtrsm_LLT_dtrsm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    ...
    .belongs_to = &flow_of_dtrsm_LLT_dgemm_for_D,
};
...
static const parsec_flow_t flow_of_dtrsm_LLT_dgemm_for_D = {
    ...
    .flow_index = 2,
    .flow_datatype_mask = 0x0,
    .dep_in = {&flow_of_dtrsm_LLT_dgemm_for_D_dep1_atline_102},
    .dep_out = {NULL}
};

static void
iterate_successors_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es, const __parsec_dtrsm_LLT_dgemm_task_t * this_task,
				      uint32_t action_mask, parsec_ontask_function_t * ontask, void *ontask_arg)
{
    ...
    __parsec_dtrsm_LLT_dgemm_parsec_assignment_t __jdf2c__tmp_locals = *(__parsec_dtrsm_LLT_dgemm_parsec_assignment_t *) & this_task->locals;	/* copy of this_task locals in R/W mode to manage local definitions */
    ...
    rank_src = rank_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - m));
#endif
    if (action_mask & 0x3) {	/* Flow of data E */
	data.data = this_task->data._f_E.data_out;
	data.arena = __parsec_tp->super.arenas[PARSEC_dtrsm_LLT_VECTOR_ARENA];
	data.layout = data.arena->opaque_dtt;
	data.count = 1;
	data.displ = 0;
	if (action_mask & 0x1) {
	    if (((1 + k) == m)) {
		__parsec_dtrsm_LLT_dtrsm_task_t *ncc = (__parsec_dtrsm_LLT_dtrsm_task_t *) & nc;
		nc.task_class = __parsec_tp->super.super.task_classes_array[dtrsm_LLT_dtrsm.task_class_id];
		const int dtrsm_k = m;
		if ((dtrsm_k >= (0)) && (dtrsm_k <= ((descB->mt - 1)))) {
...
#if defined(DISTRIBUTED)
			rank_dst =
			    rank_of_descA(((descB->mt - 1) - ncc->locals.k.value),
					  ((descB->mt - 1) - ncc->locals.k.value));
			...
			if (PARSEC_ITERATE_STOP ==
			    ontask(es, &nc, (const parsec_task_t *) this_task,
				   &flow_of_dtrsm_LLT_dgemm_for_E_dep3_atline_105, &data, rank_src, rank_dst, vpid_dst,
				   ontask_arg))
			    return;
		    }
		}
	...
	if (action_mask & 0x2) {
	    if ((m >= (k + 2))) {
		__parsec_dtrsm_LLT_dgemm_task_t *ncc = (__parsec_dtrsm_LLT_dgemm_task_t *) & nc;
		nc.task_class = __parsec_tp->super.super.task_classes_array[dtrsm_LLT_dgemm.task_class_id];
		...
		if ((dgemm_k >= (0)) && (dgemm_k <= ((descB->mt - 2)))) {
			...
			if ((dgemm_n == (0))) {
			    ...
			    if (PARSEC_ITERATE_STOP ==
				ontask(es, &nc, (const parsec_task_t *) this_task,
				       &flow_of_dtrsm_LLT_dgemm_for_E_dep4_atline_106, &data, rank_src, rank_dst,
				       vpid_dst, ontask_arg))
				return;
			}
		    }
...
static void
iterate_predecessors_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es,
					const __parsec_dtrsm_LLT_dgemm_task_t * this_task, uint32_t action_mask,
					parsec_ontask_function_t * ontask, void *ontask_arg)
{
	...
	if (action_mask & 0x4) {
		...
		if ((dgemm_k >= (0)) && (dgemm_k <= ((descB->mt - 2)))) {
			...
			if ((dgemm_n == (0))) {
			    ...
			    if (PARSEC_ITERATE_STOP ==
				ontask(es, &nc, (const parsec_task_t *) this_task,
				       &flow_of_dtrsm_LLT_dgemm_for_E_dep1_atline_103, &data, rank_src, rank_dst,
				       vpid_dst, ontask_arg))
				return;
			}
		    }
	...
	if (action_mask & 0x8) {
	    if ((0 == k)) {
		__parsec_dtrsm_LLT_read_B0_task_t *ncc = (__parsec_dtrsm_LLT_read_B0_task_t *) & nc;
		nc.task_class = __parsec_tp->super.super.task_classes_array[dtrsm_LLT_read_B0.task_class_id];
		const int read_B0_k = m;
		if ((read_B0_k >= (0)) && (read_B0_k <= ((descB->mt - 1)))) {
		    assert(&nc.locals[0].value == &ncc->locals.k.value);
		    ncc->locals.k.value = read_B0_k;
#if defined(DISTRIBUTED)
		    rank_dst = rank_of_descB(((descB->mt - 1) - ncc->locals.k.value), 0);
		    if ((NULL != es) && (rank_dst == es->virtual_process->parsec_context->my_rank))
#endif /* DISTRIBUTED */
			vpid_dst =
			    ((parsec_data_collection_t *) __parsec_tp->super._g_descB)->
			    vpid_of((parsec_data_collection_t *) __parsec_tp->super._g_descB,
				    ((descB->mt - 1) - ncc->locals.k.value), 0);
		    nc.priority = __parsec_tp->super.super.priority;
		    RELEASE_DEP_OUTPUT(es, "E", this_task, "B", &nc, rank_src, rank_dst, &data);
		    if (PARSEC_ITERATE_STOP ==
			ontask(es, &nc, (const parsec_task_t *) this_task,
			       &flow_of_dtrsm_LLT_dgemm_for_E_dep2_atline_104, &data, rank_src, rank_dst, vpid_dst,
			       ontask_arg))
			return;
		}
	    }
...
#if defined(DISTRIBUTED)
		    rank_dst =
			rank_of_descA(((descB->mt - 1) - ncc->locals.k.value), ((descB->mt - 1) - ncc->locals.k.value));
		    if ((NULL != es) && (rank_dst == es->virtual_process->parsec_context->my_rank))
#endif /* DISTRIBUTED */
			vpid_dst =
			    ((parsec_data_collection_t *) __parsec_tp->super._g_descA)->
			    vpid_of((parsec_data_collection_t *) __parsec_tp->super._g_descA,
				    ((descB->mt - 1) - ncc->locals.k.value), ((descB->mt - 1) - ncc->locals.k.value));
		    nc.priority = __parsec_tp->super.super.priority;
		    RELEASE_DEP_OUTPUT(es, "D", this_task, "B", &nc, rank_src, rank_dst, &data);
		    if (PARSEC_ITERATE_STOP ==
			ontask(es, &nc, (const parsec_task_t *) this_task,
			       &flow_of_dtrsm_LLT_dgemm_for_D_dep1_atline_102, &data, rank_src, rank_dst, vpid_dst,
			       ontask_arg))
			return;
		}
	    }
...
static int release_deps_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dgemm_task_t * this_task,
					   uint32_t action_mask, parsec_remote_deps_t * deps)
{
	...
	iterate_successors_of_dtrsm_LLT_dgemm(es, this_task, action_mask, parsec_release_dep_fct, &arg);
...
static int data_lookup_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dgemm_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
  ...
  /** Lookup the input data, and store them in the context if any */
	...
	entry = NULL;
	if ((k >= 1)) {
	    __parsec_dtrsm_LLT_dgemm_parsec_assignment_t *target_locals =
		(__parsec_dtrsm_LLT_dgemm_parsec_assignment_t *) & generic_locals;
	    ...
	    ACQUIRE_FLOW(this_task, "E", &dtrsm_LLT_dgemm, "E", target_locals, chunk);
	    this_task->data._f_E.data_out = chunk;
	} else if ((0 == k)) {
	    __parsec_dtrsm_LLT_read_B0_parsec_assignment_t *target_locals =
		(__parsec_dtrsm_LLT_read_B0_parsec_assignment_t *) & generic_locals;
	    ...
	    ACQUIRE_FLOW(this_task, "E", &dtrsm_LLT_read_B0, "B", target_locals, chunk);
	    this_task->data._f_E.data_out = chunk;
	}
	...
	parsec_prof_grapher_data_input(chunk->original, (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dgemm_for_E, 0);
#endif
    }
	...
	entry = NULL;
	chunk = parsec_data_get_copy(data_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - m)), target_device);
	PARSEC_OBJ_RETAIN(chunk);
	this_task->data._f_C.data_out = chunk;
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(data_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - m)),
				       (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dgemm_for_C, 1);
#endif
...
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(chunk->original, (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dgemm_for_C, 0);
#endif
    }
	...
	entry = NULL;
	__parsec_dtrsm_LLT_dtrsm_parsec_assignment_t *target_locals =
	    (__parsec_dtrsm_LLT_dtrsm_parsec_assignment_t *) & generic_locals;
	...
	ACQUIRE_FLOW(this_task, "D", &dtrsm_LLT_dtrsm, "B", target_locals, chunk);
...
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(chunk->original, (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dgemm_for_D, 0);
#endif
    }
...
static int datatype_lookup_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es,
					      const __parsec_dtrsm_LLT_dgemm_task_t * this_task, uint32_t * flow_mask,
					      parsec_dep_data_description_t * data)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
	...
	if ((*flow_mask) & 0x2U) {	/* Flow C */
		...
		data->arena = __parsec_tp->super.arenas[PARSEC_dtrsm_LLT_FULL_ARENA];
		data->layout = data->arena->opaque_dtt;
		data->count = 1;
		data->displ = 0;
		(*flow_mask) &= ~0x2U;
...
static int hook_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dgemm_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    (void) es;
    (void) __parsec_tp;
    const int k = this_task->locals.k.value;
...
#line 110 "dtrsm_LLT.jdf"
	...
	CORE_dgemm(trans, dplasmaNoTrans,
		   descB->mb, tempnn, tempkm, -1.0, C /* descA((descB->mt-1)-k,(descB->mt-1)-m) */ , lda,
	...
	printlog("CORE_dgemm(%d, %d, %d)\n"
		 ...
		 k, m, n, (descB->mt - 1) - k, (descB->mt - 1) - m, C, (descB->mt - 1) - k, n, D, (descB->mt - 1) - m,
		 n, E);
    }

#line 1365 "dtrsm_LLT.c"
/*-----                              END OF dgemm BODY                                -----*/
...
static int complete_hook_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dgemm_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
#if defined(DISTRIBUTED)
  ...
  /** If not working on distributed, there is no risk that data is not in place */
    ...
    release_deps_of_dtrsm_LLT_dgemm(es, this_task, PARSEC_ACTION_RELEASE_REMOTE_DEPS | PARSEC_ACTION_RELEASE_LOCAL_DEPS | PARSEC_ACTION_RELEASE_LOCAL_REFS | 0x3,	/* mask of all dep_index */
				    NULL);
    return PARSEC_HOOK_RETURN_DONE;
}

static parsec_hook_return_t release_task_of_dtrsm_LLT_dgemm(parsec_execution_stream_t * es, parsec_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
...
static char *dtrsm_LLT_dtrsm_LLT_dgemm_internal_init_deps_key_functions_key_print(char *buffer, size_t buffer_size,
...
static parsec_key_fn_t dtrsm_LLT_dtrsm_LLT_dgemm_internal_init_deps_key_functions = {
    .key_equal = parsec_hash_table_generic_64bits_key_equal,
    .key_print = dtrsm_LLT_dtrsm_LLT_dgemm_internal_init_deps_key_functions_key_print,
    .key_hash = parsec_hash_table_generic_64bits_key_hash
};

/* Needs: min-max count-tasks iterate */
static int dtrsm_LLT_dgemm_internal_init(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dgemm_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    ...
    __parsec_dtrsm_LLT_dgemm_parsec_assignment_t assignments;
    int32_t k, m, n;
    ...
    PARSEC_AYU_REGISTER_TASK(&dtrsm_LLT_dgemm);
    ...
    parsec_hash_table_init(__parsec_tp->super.super.dependencies_array[3],
			   ...
			   dtrsm_LLT_dtrsm_LLT_dgemm_internal_init_deps_key_functions, this_task->taskpool);
    __parsec_tp->repositories[3] =
	data_repo_create_nothreadsafe(nb_tasks, __jdf2c_key_fns_dgemm, (parsec_taskpool_t *) __parsec_tp, 3);
    (void) saved_nb_tasks;
    (void) __k_min;
    (void) __k_max;
    (void) __m_min;
...
static const __parsec_chore_t __dtrsm_LLT_dgemm_chores[] = {
    {.type = PARSEC_DEV_CPU,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) hook_of_dtrsm_LLT_dgemm},
...
static const parsec_task_class_t dtrsm_LLT_dgemm = {
    ...
    .nb_parameters = 3,
    .nb_locals = 3,
    .params = {&symb_dtrsm_LLT_dgemm_k, &symb_dtrsm_LLT_dgemm_m, &symb_dtrsm_LLT_dgemm_n, NULL},
    .locals = {&symb_dtrsm_LLT_dgemm_k, &symb_dtrsm_LLT_dgemm_m, &symb_dtrsm_LLT_dgemm_n, NULL},
    .data_affinity = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_dgemm,
    .initial_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_dgemm,
    .final_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_dgemm,
    .priority = NULL,
    .properties = properties_of_dtrsm_LLT_dgemm,
...
#error Too many write flows for task dgemm
#endif /* MAX_PARAM_COUNT */
    .in = {&flow_of_dtrsm_LLT_dgemm_for_E, &flow_of_dtrsm_LLT_dgemm_for_C, &flow_of_dtrsm_LLT_dgemm_for_D, NULL},
    .out = {&flow_of_dtrsm_LLT_dgemm_for_E, NULL},
    ...
    .incarnations = __dtrsm_LLT_dgemm_chores,
    .find_deps = parsec_hash_find_deps,
    .iterate_successors = (parsec_traverse_function_t *) iterate_successors_of_dtrsm_LLT_dgemm,
    .iterate_predecessors = (parsec_traverse_function_t *) iterate_predecessors_of_dtrsm_LLT_dgemm,
    .release_deps = (parsec_release_deps_t *) release_deps_of_dtrsm_LLT_dgemm,
    .prepare_input = (parsec_hook_t *) data_lookup_of_dtrsm_LLT_dgemm,
    .prepare_output = (parsec_hook_t *) NULL,
    .get_datatype = (parsec_datatype_lookup_t *) datatype_lookup_of_dtrsm_LLT_dgemm,
    .complete_execution = (parsec_hook_t *) complete_hook_of_dtrsm_LLT_dgemm,
    .release_task = &release_task_of_dtrsm_LLT_dgemm,
#if defined(PARSEC_SIM)
...
static inline int32_t minexpr_of_symb_dtrsm_LLT_write_B0_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t *
							       __parsec_tp,
							       const __parsec_dtrsm_LLT_write_B0_parsec_assignment_t *
							       locals)
{
    (void) __parsec_tp;
    (void) locals;
...
static const parsec_expr_t minexpr_of_symb_dtrsm_LLT_write_B0_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) minexpr_of_symb_dtrsm_LLT_write_B0_k_fct}
		      }
};

static inline int maxexpr_of_symb_dtrsm_LLT_write_B0_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							   const __parsec_dtrsm_LLT_write_B0_parsec_assignment_t *
							   locals)
{
    const int k = locals->k.value;
...
static const parsec_expr_t maxexpr_of_symb_dtrsm_LLT_write_B0_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) maxexpr_of_symb_dtrsm_LLT_write_B0_k_fct}
		      }
};
static const parsec_symbol_t symb_dtrsm_LLT_write_B0_k = {.name = "k",.context_index = 0,.min =
	&minexpr_of_symb_dtrsm_LLT_write_B0_k,.max = &maxexpr_of_symb_dtrsm_LLT_write_B0_k,.cst_inc = 1,.expr_inc =
	NULL,.flags = 0x0 };

static inline int affinity_of_dtrsm_LLT_write_B0(__parsec_dtrsm_LLT_write_B0_task_t * this_task,
						 parsec_data_ref_t * ref)
{
    ...
    /* Compute data key */
    ref->key = ref->dc->data_key(ref->dc, ((descB->mt - 1) - k), 0);
    return 1;
}

static const parsec_property_t properties_of_dtrsm_LLT_write_B0[1] = {
    {.name = NULL,.expr = NULL}
};

static const parsec_dep_t flow_of_dtrsm_LLT_write_B0_for_B_dep1_atline_84 = {
    .cond = NULL,		/*  */
    .ctl_gather_nb = NULL,
    .task_class_id = 0,		/* dtrsm_LLT_dtrsm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    ...
    .dep_index = 0,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_write_B0_for_B,
};

static parsec_data_t *flow_of_dtrsm_LLT_write_B0_for_B_dep2_atline_85_direct_access(const
										    __parsec_dtrsm_LLT_internal_taskpool_t
										    * __parsec_tp,
										    const
										    ...
										    * assignments)
{
    ...
    /* Silence Warnings: should look into parameters to know what variables are useful */
    (void) k;
    if (__parsec_tp->super.super.context->my_rank == (int32_t) rank_of_descB(((descB->mt - 1) - k), 0))
	return data_of_descB(((descB->mt - 1) - k), 0);
    return NULL;
}

static const parsec_dep_t flow_of_dtrsm_LLT_write_B0_for_B_dep2_atline_85 = {
    .cond = NULL,		/*  */
    .ctl_gather_nb = NULL,
    .task_class_id = PARSEC_LOCAL_DATA_TASK_CLASS_ID,	/* dtrsm_LLT_descB */
    .direct_data = (parsec_data_lookup_func_t) & flow_of_dtrsm_LLT_write_B0_for_B_dep2_atline_85_direct_access,
    .dep_index = 0,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_write_B0_for_B,
};
...
static const parsec_flow_t flow_of_dtrsm_LLT_write_B0_for_B = {
    ...
    .flow_index = 0,
    .flow_datatype_mask = 0x1,
    .dep_in = {&flow_of_dtrsm_LLT_write_B0_for_B_dep1_atline_84},
    .dep_out = {&flow_of_dtrsm_LLT_write_B0_for_B_dep2_atline_85}
};

static void
iterate_predecessors_of_dtrsm_LLT_write_B0(parsec_execution_stream_t * es,
					   const __parsec_dtrsm_LLT_write_B0_task_t * this_task, uint32_t action_mask,
					   parsec_ontask_function_t * ontask, void *ontask_arg)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    parsec_task_t nc;		/* generic placeholder for locals */
    parsec_dep_data_description_t data;
    __parsec_dtrsm_LLT_write_B0_parsec_assignment_t __jdf2c__tmp_locals = *(__parsec_dtrsm_LLT_write_B0_parsec_assignment_t *) & this_task->locals;	/* copy of this_task locals in R/W mode to manage local definitions */
    int vpid_dst = -1, rank_src = 0, rank_dst = 0;
    const int k = __jdf2c__tmp_locals.k.value;
    (void) k;
    (void) rank_src;
    ...
    nc.chore_id = 0;
#if defined(DISTRIBUTED)
    rank_src = rank_of_descB(((descB->mt - 1) - k), 0);
#endif
	...
	if (action_mask & 0x1) {
	    __parsec_dtrsm_LLT_dtrsm_task_t *ncc = (__parsec_dtrsm_LLT_dtrsm_task_t *) & nc;
	    nc.task_class = __parsec_tp->super.super.task_classes_array[dtrsm_LLT_dtrsm.task_class_id];
	    const int dtrsm_k = ((descB->mt - 1) - k);
	    if ((dtrsm_k >= (0)) && (dtrsm_k <= ((descB->mt - 1)))) {
		assert(&nc.locals[0].value == &ncc->locals.k.value);
		ncc->locals.k.value = dtrsm_k;
		const int dtrsm_n = 0;
		if ((dtrsm_n == (0))) {
		    ...
		    if (PARSEC_ITERATE_STOP ==
			ontask(es, &nc, (const parsec_task_t *) this_task,
			       &flow_of_dtrsm_LLT_write_B0_for_B_dep1_atline_84, &data, rank_src, rank_dst, vpid_dst,
			       ontask_arg))
			return;
		}
	    }
...
static int release_deps_of_dtrsm_LLT_write_B0(parsec_execution_stream_t * es,
					      __parsec_dtrsm_LLT_write_B0_task_t * this_task, uint32_t action_mask,
...
static int data_lookup_of_dtrsm_LLT_write_B0(parsec_execution_stream_t * es,
					     __parsec_dtrsm_LLT_write_B0_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    parsec_assignment_t generic_locals[MAX_PARAM_COUNT];	/* generic task locals */
  ...
  /** Lookup the input data, and store them in the context if any */
	...
	__parsec_dtrsm_LLT_dtrsm_parsec_assignment_t *target_locals =
	    (__parsec_dtrsm_LLT_dtrsm_parsec_assignment_t *) & generic_locals;
	const int dtrsmk = target_locals->k.value = ((descB->mt - 1) - k);
	(void) dtrsmk;
	...
	ACQUIRE_FLOW(this_task, "B", &dtrsm_LLT_dtrsm, "B", target_locals, chunk);
...
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(chunk->original, (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_write_B0_for_B,
				       0);
#endif
    }
  /** Generate profiling information */
...
static int datatype_lookup_of_dtrsm_LLT_write_B0(parsec_execution_stream_t * es,
						 const __parsec_dtrsm_LLT_write_B0_task_t * this_task,
...
static int hook_of_dtrsm_LLT_write_B0(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_write_B0_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    (void) es;
...
/*-----                                write_B0 BODY                                  -----*/
    ...
    PARSEC_TASK_PROF_TRACE(es->es_profile,
			   this_task->taskpool->profiling_array[2 * this_task->task_class->task_class_id],
			   (parsec_task_t *) this_task);
#line 87 "dtrsm_LLT.jdf"
    {
	/* nothing */
    }

#line 1977 "dtrsm_LLT.c"
...
static int complete_hook_of_dtrsm_LLT_write_B0(parsec_execution_stream_t * es,
					       __parsec_dtrsm_LLT_write_B0_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
  ...
  /** If not working on distributed, there is no risk that data is not in place */
    if ((NULL != this_task->data._f_B.data_out)
	&& (this_task->data._f_B.data_out->original != data_of_descB(((descB->mt - 1) - k), 0))) {
	parsec_dep_data_description_t data;
	data.data = this_task->data._f_B.data_out;
	data.arena = __parsec_tp->super.arenas[PARSEC_dtrsm_LLT_VECTOR_ARENA];
	data.layout = data.arena->opaque_dtt;
	...
	parsec_remote_dep_memcpy(es,
				 this_task->taskpool,
				 parsec_data_get_copy(data_of_descB(((descB->mt - 1) - k), 0), 0),
				 this_task->data._f_B.data_out, &data);
    }
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
    parsec_prof_grapher_data_output((parsec_task_t *) this_task, data_of_descB(((descB->mt - 1) - k), 0),
				    &flow_of_dtrsm_LLT_write_B0_for_B);
#endif
    (void) k;
    ...
    parsec_prof_grapher_task((parsec_task_t *) this_task, es->th_id, es->virtual_process->vp_id,
			     __jdf2c_key_fns_write_B0.key_hash(this_task->task_class->
							       make_key((parsec_taskpool_t *) this_task->taskpool,
									((parsec_task_t *) this_task)->locals), NULL));
#endif /* defined(PARSEC_PROF_GRAPHER) */
    release_deps_of_dtrsm_LLT_write_B0(es, this_task, PARSEC_ACTION_RELEASE_REMOTE_DEPS | PARSEC_ACTION_RELEASE_LOCAL_DEPS | PARSEC_ACTION_RELEASE_LOCAL_REFS | 0x1,	/* mask of all dep_index */
				       NULL);
    return PARSEC_HOOK_RETURN_DONE;
}

static parsec_hook_return_t release_task_of_dtrsm_LLT_write_B0(parsec_execution_stream_t * es,
							       parsec_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
...
static char *dtrsm_LLT_dtrsm_LLT_write_B0_internal_init_deps_key_functions_key_print(char *buffer, size_t buffer_size,
										     parsec_key_t __parsec_key_,
										     void *user_data)
{
    uint64_t __parsec_key = (uint64_t) (uintptr_t) __parsec_key_;
...
static parsec_key_fn_t dtrsm_LLT_dtrsm_LLT_write_B0_internal_init_deps_key_functions = {
    .key_equal = parsec_hash_table_generic_64bits_key_equal,
    .key_print = dtrsm_LLT_dtrsm_LLT_write_B0_internal_init_deps_key_functions_key_print,
    .key_hash = parsec_hash_table_generic_64bits_key_hash
};

/* Needs: min-max count-tasks iterate */
static int dtrsm_LLT_write_B0_internal_init(parsec_execution_stream_t * es,
					    __parsec_dtrsm_LLT_write_B0_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    ...
    int32_t __k_min = 0x7fffffff, __k_max = 0;
    int32_t __jdf2c_k_min = 0x7fffffff, __jdf2c_k_max = 0;
    __parsec_dtrsm_LLT_write_B0_parsec_assignment_t assignments;
    int32_t k;
    int32_t __jdf2c_k_start, __jdf2c_k_end, __jdf2c_k_inc;
#if defined(PARSEC_PROF_TRACE) && defined(PARSEC_PROF_TRACE_PTG_INTERNAL_INIT)
    PARSEC_PROFILING_TRACE(es->es_profile,
    ...
    PARSEC_AYU_REGISTER_TASK(&dtrsm_LLT_write_B0);
    ...
    parsec_hash_table_init(__parsec_tp->super.super.dependencies_array[2],
			   offsetof(parsec_hashable_dependency_t, ht_item), 10,
			   dtrsm_LLT_dtrsm_LLT_write_B0_internal_init_deps_key_functions, this_task->taskpool);
...
static const __parsec_chore_t __dtrsm_LLT_write_B0_chores[] = {
    {.type = PARSEC_DEV_CPU,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) hook_of_dtrsm_LLT_write_B0},
    {.type = PARSEC_DEV_NONE,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) NULL},	/* End marker */
};
...
static const parsec_task_class_t dtrsm_LLT_write_B0 = {
    ...
    .nb_parameters = 1,
    .nb_locals = 1,
    .params = {&symb_dtrsm_LLT_write_B0_k, NULL},
    .locals = {&symb_dtrsm_LLT_write_B0_k, NULL},
    .data_affinity = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_write_B0,
    .initial_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_write_B0,
    .final_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_write_B0,
    .priority = NULL,
    .properties = properties_of_dtrsm_LLT_write_B0,
...
#error Too many write flows for task write_B0
#endif /* MAX_PARAM_COUNT */
    .in = {&flow_of_dtrsm_LLT_write_B0_for_B, NULL},
    .out = {&flow_of_dtrsm_LLT_write_B0_for_B, NULL},
    ...
    .fini = (parsec_hook_t *) NULL,
    .incarnations = __dtrsm_LLT_write_B0_chores,
    .find_deps = parsec_hash_find_deps,
    .iterate_successors = (parsec_traverse_function_t *) NULL,
    .iterate_predecessors = (parsec_traverse_function_t *) iterate_predecessors_of_dtrsm_LLT_write_B0,
    .release_deps = (parsec_release_deps_t *) release_deps_of_dtrsm_LLT_write_B0,
    .prepare_input = (parsec_hook_t *) data_lookup_of_dtrsm_LLT_write_B0,
    .prepare_output = (parsec_hook_t *) NULL,
    .get_datatype = (parsec_datatype_lookup_t *) datatype_lookup_of_dtrsm_LLT_write_B0,
    .complete_execution = (parsec_hook_t *) complete_hook_of_dtrsm_LLT_write_B0,
    .release_task = &release_task_of_dtrsm_LLT_write_B0,
#if defined(PARSEC_SIM)
    .sim_cost_fct = (parsec_sim_cost_fct_t *) NULL,
#endif
};
...
static inline int32_t minexpr_of_symb_dtrsm_LLT_read_B0_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t *
							      __parsec_tp,
							      const __parsec_dtrsm_LLT_read_B0_parsec_assignment_t *
							      locals)
{
    (void) __parsec_tp;
    (void) locals;
...
static const parsec_expr_t minexpr_of_symb_dtrsm_LLT_read_B0_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) minexpr_of_symb_dtrsm_LLT_read_B0_k_fct}
		      }
};

static inline int maxexpr_of_symb_dtrsm_LLT_read_B0_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							  const __parsec_dtrsm_LLT_read_B0_parsec_assignment_t * locals)
{
    const int k = locals->k.value;
    (void) k;
...
static const parsec_expr_t maxexpr_of_symb_dtrsm_LLT_read_B0_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) maxexpr_of_symb_dtrsm_LLT_read_B0_k_fct}
		      }
};
static const parsec_symbol_t symb_dtrsm_LLT_read_B0_k = {.name = "k",.context_index = 0,.min =
	&minexpr_of_symb_dtrsm_LLT_read_B0_k,.max = &maxexpr_of_symb_dtrsm_LLT_read_B0_k,.cst_inc = 1,.expr_inc =
	NULL,.flags = 0x0 };

static inline int affinity_of_dtrsm_LLT_read_B0(__parsec_dtrsm_LLT_read_B0_task_t * this_task, parsec_data_ref_t * ref)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    const int k = this_task->locals.k.value;
...
static const parsec_property_t properties_of_dtrsm_LLT_read_B0[1] = {
    {.name = NULL,.expr = NULL}
};

static parsec_data_t *flow_of_dtrsm_LLT_read_B0_for_B_dep1_atline_67_direct_access(const
										   __parsec_dtrsm_LLT_internal_taskpool_t
										   * __parsec_tp,
										   const
...
static const parsec_dep_t flow_of_dtrsm_LLT_read_B0_for_B_dep1_atline_67 = {
    .cond = NULL,		/*  */
    .ctl_gather_nb = NULL,
    .task_class_id = PARSEC_LOCAL_DATA_TASK_CLASS_ID,	/* dtrsm_LLT_descB */
    .direct_data = (parsec_data_lookup_func_t) & flow_of_dtrsm_LLT_read_B0_for_B_dep1_atline_67_direct_access,
    .dep_index = 0,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_read_B0_for_B,
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep2_atline_68_fct(const
										      __parsec_dtrsm_LLT_internal_taskpool_t
										      * __parsec_tp,
										      const
										      __parsec_dtrsm_LLT_read_B0_parsec_assignment_t
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep2_atline_68 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep2_atline_68_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_read_B0_for_B_dep2_atline_68 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep2_atline_68,	/* (k == 0) */
    .ctl_gather_nb = NULL,
    .task_class_id = 0,		/* dtrsm_LLT_dtrsm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_dtrsm_for_B,
...
static inline int expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep3_atline_69_fct(const
										      __parsec_dtrsm_LLT_internal_taskpool_t
										      * __parsec_tp,
										      const
										      __parsec_dtrsm_LLT_read_B0_parsec_assignment_t
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep3_atline_69 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep3_atline_69_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_read_B0_for_B_dep3_atline_69 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_read_B0_for_B_dep3_atline_69,	/* (k >= 1) */
    .ctl_gather_nb = NULL,
    .task_class_id = 3,		/* dtrsm_LLT_dgemm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_dgemm_for_E,
...
static const parsec_flow_t flow_of_dtrsm_LLT_read_B0_for_B = {
    ...
    .flow_index = 0,
    .flow_datatype_mask = 0x1,
    .dep_in = {&flow_of_dtrsm_LLT_read_B0_for_B_dep1_atline_67},
    .dep_out = {&flow_of_dtrsm_LLT_read_B0_for_B_dep2_atline_68,
		&flow_of_dtrsm_LLT_read_B0_for_B_dep3_atline_69}
};

static void
iterate_successors_of_dtrsm_LLT_read_B0(parsec_execution_stream_t * es,
					const __parsec_dtrsm_LLT_read_B0_task_t * this_task, uint32_t action_mask,
					parsec_ontask_function_t * ontask, void *ontask_arg)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    parsec_task_t nc;		/* generic placeholder for locals */
    parsec_dep_data_description_t data;
    __parsec_dtrsm_LLT_read_B0_parsec_assignment_t __jdf2c__tmp_locals = *(__parsec_dtrsm_LLT_read_B0_parsec_assignment_t *) & this_task->locals;	/* copy of this_task locals in R/W mode to manage local definitions */
    int vpid_dst = -1, rank_src = 0, rank_dst = 0;
    const int k = __jdf2c__tmp_locals.k.value;
    (void) k;
    (void) rank_src;
	...
	if (action_mask & 0x1) {
		...
		if ((dtrsm_k >= (0)) && (dtrsm_k <= ((descB->mt - 1)))) {
			...
			if (PARSEC_ITERATE_STOP ==
			    ontask(es, &nc, (const parsec_task_t *) this_task,
				   &flow_of_dtrsm_LLT_read_B0_for_B_dep2_atline_68, &data, rank_src, rank_dst, vpid_dst,
				   ontask_arg))
			    return;
		    }
		}
	...
	if (action_mask & 0x2) {
		...
		if ((dgemm_k >= (0)) && (dgemm_k <= ((descB->mt - 2)))) {
			...
			if ((dgemm_n == (0))) {
			    ...
			    if (PARSEC_ITERATE_STOP ==
				ontask(es, &nc, (const parsec_task_t *) this_task,
				       &flow_of_dtrsm_LLT_read_B0_for_B_dep3_atline_69, &data, rank_src, rank_dst,
				       vpid_dst, ontask_arg))
				return;
			}
		    }
...
static int release_deps_of_dtrsm_LLT_read_B0(parsec_execution_stream_t * es,
					     __parsec_dtrsm_LLT_read_B0_task_t * this_task, uint32_t action_mask,
					     parsec_remote_deps_t * deps)
{
    PARSEC_PINS(es, RELEASE_DEPS_BEGIN, (parsec_task_t *) this_task); {
	const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	...
	iterate_successors_of_dtrsm_LLT_read_B0(es, this_task, action_mask, parsec_release_dep_fct, &arg);
...
static int data_lookup_of_dtrsm_LLT_read_B0(parsec_execution_stream_t * es,
					    __parsec_dtrsm_LLT_read_B0_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    parsec_assignment_t generic_locals[MAX_PARAM_COUNT];	/* generic task locals */
  ...
  /** Lookup the input data, and store them in the context if any */
    ...
    if (NULL == (chunk = this_task->data._f_B.data_in)) {	/* flow B */
	entry = NULL;
	chunk = parsec_data_get_copy(data_of_descB(((descB->mt - 1) - k), 0), target_device);
	PARSEC_OBJ_RETAIN(chunk);
	this_task->data._f_B.data_out = chunk;
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(data_of_descB(((descB->mt - 1) - k), 0), (parsec_task_t *) this_task,
				       &flow_of_dtrsm_LLT_read_B0_for_B, 1);
#endif
	this_task->data._f_B.data_in = chunk;	/* flow B */
	this_task->data._f_B.data_repo = entry;
    } else {
	this_task->data._f_B.data_out = parsec_data_get_copy(chunk->original, target_device);
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(chunk->original, (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_read_B0_for_B,
				       0);
#endif
    }
  /** Generate profiling information */
...
static int datatype_lookup_of_dtrsm_LLT_read_B0(parsec_execution_stream_t * es,
						const __parsec_dtrsm_LLT_read_B0_task_t * this_task,
						uint32_t * flow_mask, parsec_dep_data_description_t * data)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
...
static int hook_of_dtrsm_LLT_read_B0(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_read_B0_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    (void) es;
...
/*-----                                  read_B0 BODY                                  -----*/
    ...
    PARSEC_TASK_PROF_TRACE(es->es_profile,
			   this_task->taskpool->profiling_array[2 * this_task->task_class->task_class_id],
			   (parsec_task_t *) this_task);
#line 71 "dtrsm_LLT.jdf"
    {
	/* nothing */
    }

#line 2704 "dtrsm_LLT.c"
/*-----                              END OF read_B0 BODY                              -----*/
...
static int complete_hook_of_dtrsm_LLT_read_B0(parsec_execution_stream_t * es,
					      __parsec_dtrsm_LLT_read_B0_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    ...
    parsec_prof_grapher_task((parsec_task_t *) this_task, es->th_id, es->virtual_process->vp_id,
			     __jdf2c_key_fns_read_B0.key_hash(this_task->task_class->
							      make_key((parsec_taskpool_t *) this_task->taskpool,
								       ((parsec_task_t *) this_task)->locals), NULL));
#endif /* defined(PARSEC_PROF_GRAPHER) */
    release_deps_of_dtrsm_LLT_read_B0(es, this_task, PARSEC_ACTION_RELEASE_REMOTE_DEPS | PARSEC_ACTION_RELEASE_LOCAL_DEPS | PARSEC_ACTION_RELEASE_LOCAL_REFS | 0x3,	/* mask of all dep_index */
				      NULL);
    return PARSEC_HOOK_RETURN_DONE;
}

static parsec_hook_return_t release_task_of_dtrsm_LLT_read_B0(parsec_execution_stream_t * es, parsec_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
...
static char *dtrsm_LLT_dtrsm_LLT_read_B0_internal_init_deps_key_functions_key_print(char *buffer, size_t buffer_size,
										    parsec_key_t __parsec_key_,
										    void *user_data)
{
    uint64_t __parsec_key = (uint64_t) (uintptr_t) __parsec_key_;
...
static parsec_key_fn_t dtrsm_LLT_dtrsm_LLT_read_B0_internal_init_deps_key_functions = {
    .key_equal = parsec_hash_table_generic_64bits_key_equal,
    .key_print = dtrsm_LLT_dtrsm_LLT_read_B0_internal_init_deps_key_functions_key_print,
    .key_hash = parsec_hash_table_generic_64bits_key_hash
};

/* Needs: min-max count-tasks iterate */
static int dtrsm_LLT_read_B0_internal_init(parsec_execution_stream_t * es,
					   __parsec_dtrsm_LLT_read_B0_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    ...
    int32_t __k_min = 0x7fffffff, __k_max = 0;
    int32_t __jdf2c_k_min = 0x7fffffff, __jdf2c_k_max = 0;
    __parsec_dtrsm_LLT_read_B0_parsec_assignment_t assignments;
    int32_t k;
    int32_t __jdf2c_k_start, __jdf2c_k_end, __jdf2c_k_inc;
#if defined(PARSEC_PROF_TRACE) && defined(PARSEC_PROF_TRACE_PTG_INTERNAL_INIT)
    PARSEC_PROFILING_TRACE(es->es_profile,
    ...
    PARSEC_AYU_REGISTER_TASK(&dtrsm_LLT_read_B0);
    ...
    parsec_hash_table_init(__parsec_tp->super.super.dependencies_array[1],
			   offsetof(parsec_hashable_dependency_t, ht_item), 10,
			   dtrsm_LLT_dtrsm_LLT_read_B0_internal_init_deps_key_functions, this_task->taskpool);
...
static int __jdf2c_startup_read_B0(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_read_B0_task_t * this_task)
{
    __parsec_dtrsm_LLT_read_B0_task_t *new_task;
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
	...
	if (!read_B0_pred(k))
	    continue;
	__parsec_dtrsm_LLT_read_B0_parsec_assignment_t __jdf2c__tmp_locals =
	    *(__parsec_dtrsm_LLT_read_B0_parsec_assignment_t *) & this_task->locals;
	...
	if (NULL != ((parsec_data_collection_t *) __parsec_tp->super._g_descB)->vpid_of) {
	    vpid =
		((parsec_data_collection_t *) __parsec_tp->super._g_descB)->
		vpid_of((parsec_data_collection_t *) __parsec_tp->super._g_descB, ((descB->mt - 1) - k), 0);
	    assert(context->nb_vp >= vpid);
	}
	new_task =
	    (__parsec_dtrsm_LLT_read_B0_task_t *) parsec_thread_mempool_allocate(context->virtual_processes[vpid]->
	...
	new_task->task_class = __parsec_tp->super.super.task_classes_array[dtrsm_LLT_read_B0.task_class_id];
	new_task->chore_id = 0;
...
static const __parsec_chore_t __dtrsm_LLT_read_B0_chores[] = {
    {.type = PARSEC_DEV_CPU,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) hook_of_dtrsm_LLT_read_B0},
    {.type = PARSEC_DEV_NONE,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) NULL},	/* End marker */
};
...
static const parsec_task_class_t dtrsm_LLT_read_B0 = {
    ...
    .nb_parameters = 1,
    .nb_locals = 1,
    .params = {&symb_dtrsm_LLT_read_B0_k, NULL},
    .locals = {&symb_dtrsm_LLT_read_B0_k, NULL},
    .data_affinity = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_read_B0,
    .initial_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_read_B0,
    .final_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_read_B0,
    .priority = NULL,
    .properties = properties_of_dtrsm_LLT_read_B0,
...
#endif /* MAX_PARAM_COUNT */
    .in = {&flow_of_dtrsm_LLT_read_B0_for_B, NULL},
    .out = {&flow_of_dtrsm_LLT_read_B0_for_B, NULL},
    .flags = 0x0 | PARSEC_HAS_IN_IN_DEPENDENCIES | PARSEC_USE_DEPS_MASK,
    .dependencies_goal = 0x1,
    .make_key = __jdf2c_make_key_read_B0,
    .key_functions = &__jdf2c_key_fns_read_B0,
    .fini = (parsec_hook_t *) NULL,
    .incarnations = __dtrsm_LLT_read_B0_chores,
    .find_deps = parsec_hash_find_deps,
    .iterate_successors = (parsec_traverse_function_t *) iterate_successors_of_dtrsm_LLT_read_B0,
    .iterate_predecessors = (parsec_traverse_function_t *) NULL,
    .release_deps = (parsec_release_deps_t *) release_deps_of_dtrsm_LLT_read_B0,
    .prepare_input = (parsec_hook_t *) data_lookup_of_dtrsm_LLT_read_B0,
    .prepare_output = (parsec_hook_t *) NULL,
    .get_datatype = (parsec_datatype_lookup_t *) datatype_lookup_of_dtrsm_LLT_read_B0,
    .complete_execution = (parsec_hook_t *) complete_hook_of_dtrsm_LLT_read_B0,
    .release_task = &release_task_of_dtrsm_LLT_read_B0,
#if defined(PARSEC_SIM)
    .sim_cost_fct = (parsec_sim_cost_fct_t *) NULL,
#endif
};
...
static inline int32_t minexpr_of_symb_dtrsm_LLT_dtrsm_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							    const __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t * locals)
{
    (void) __parsec_tp;
    (void) locals;
    return 0;
...
static const parsec_expr_t minexpr_of_symb_dtrsm_LLT_dtrsm_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) minexpr_of_symb_dtrsm_LLT_dtrsm_k_fct}
		      }
};

static inline int maxexpr_of_symb_dtrsm_LLT_dtrsm_k_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							const __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t * locals)
{
    const int k = locals->k.value;
    (void) k;
...
static const parsec_expr_t maxexpr_of_symb_dtrsm_LLT_dtrsm_k = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) maxexpr_of_symb_dtrsm_LLT_dtrsm_k_fct}
		      }
};
static const parsec_symbol_t symb_dtrsm_LLT_dtrsm_k = {.name = "k",.context_index = 0,.min =
	&minexpr_of_symb_dtrsm_LLT_dtrsm_k,.max = &maxexpr_of_symb_dtrsm_LLT_dtrsm_k,.cst_inc = 1,.expr_inc =
	NULL,.flags = 0x0 };

static inline int32_t expr_of_symb_dtrsm_LLT_dtrsm_n_fct(const __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
							 const __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t * locals)
{
    (void) __parsec_tp;
    (void) locals;
...
static const parsec_expr_t expr_of_symb_dtrsm_LLT_dtrsm_n = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) expr_of_symb_dtrsm_LLT_dtrsm_n_fct}
		      }
};
static const parsec_symbol_t symb_dtrsm_LLT_dtrsm_n = {.name = "n",.context_index = 1,.min =
	&expr_of_symb_dtrsm_LLT_dtrsm_n,.max = &expr_of_symb_dtrsm_LLT_dtrsm_n,.cst_inc = 0,.expr_inc = NULL,.flags =
	PARSEC_SYMBOL_IS_STANDALONE };

static inline int affinity_of_dtrsm_LLT_dtrsm(__parsec_dtrsm_LLT_dtrsm_task_t * this_task, parsec_data_ref_t * ref)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    const int k = this_task->locals.k.value;
    ...
    ref->dc = (parsec_data_collection_t *) __parsec_tp->super._g_descA;
    /* Compute data key */
    ref->key = ref->dc->data_key(ref->dc, ((descB->mt - 1) - k), ((descB->mt - 1) - k));
    return 1;
}

static inline int32_t property_2_flops_of_dtrsm_LLT_dtrsm_as_expr_fct(const __parsec_dtrsm_LLT_internal_taskpool_t *
								      __parsec_tp,
								      const __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t
								      * locals)
{
    (void) __parsec_tp;
    (void) locals;
    return dtrsm_LLT_dtrsm_inline_c_expr2_line_26(__parsec_tp, locals);
}

static const parsec_expr_t property_2_flops_of_dtrsm_LLT_dtrsm_as_expr = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t) property_2_flops_of_dtrsm_LLT_dtrsm_as_expr_fct}
		      }
};

static const parsec_property_t properties_of_dtrsm_LLT_dtrsm[2] = {
    {.name = "flops",.expr = &property_2_flops_of_dtrsm_LLT_dtrsm_as_expr},
    {.name = NULL,.expr = NULL}
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep1_atline_35_fct(const
										    __parsec_dtrsm_LLT_internal_taskpool_t
										    * __parsec_tp,
										    const
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep1_atline_35 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep1_atline_35_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_dtrsm_for_B_dep1_atline_35 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep1_atline_35,	/* (0 == k) */
    .ctl_gather_nb = NULL,
    .task_class_id = 1,		/* dtrsm_LLT_read_B0 */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_read_B0_for_B,
    .dep_index = 1,
    .dep_datatype_index = 1,
    .belongs_to = &flow_of_dtrsm_LLT_dtrsm_for_B,
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep2_atline_36_fct(const
										    __parsec_dtrsm_LLT_internal_taskpool_t
										    * __parsec_tp,
										    const
										    __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep2_atline_36 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep2_atline_36_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_dtrsm_for_B_dep2_atline_36 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep2_atline_36,	/* (k >= 1) */
    .ctl_gather_nb = NULL,
    .task_class_id = 3,		/* dtrsm_LLT_dgemm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_dgemm_for_E,
...
static const parsec_dep_t flow_of_dtrsm_LLT_dtrsm_for_B_dep3_atline_37 = {
    .cond = NULL,		/*  */
    .ctl_gather_nb = NULL,
    .task_class_id = 2,		/* dtrsm_LLT_write_B0 */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_write_B0_for_B,
    .dep_index = 0,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_dtrsm_for_B,
};

static inline int expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep4_atline_38_fct(const
										    __parsec_dtrsm_LLT_internal_taskpool_t
										    * __parsec_tp,
										    const
										    __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t
...
static const parsec_expr_t expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep4_atline_38 = {
    .op = PARSEC_EXPR_OP_INLINE,
    .u_expr.v_func = {.type = PARSEC_RETURN_TYPE_INT32,	/* PARSEC_RETURN_TYPE_INT32 */
		      .func = {.inline_func_int32 =
			       (parsec_expr_op_int32_inline_func_t)
			       expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep4_atline_38_fct}
		      }
};

static const parsec_dep_t flow_of_dtrsm_LLT_dtrsm_for_B_dep4_atline_38 = {
    .cond = &expr_of_cond_for_flow_of_dtrsm_LLT_dtrsm_for_B_dep4_atline_38,	/* (descB->mt >= (2 + k)) */
    .ctl_gather_nb = NULL,
    .task_class_id = 3,		/* dtrsm_LLT_dgemm */
    .direct_data = (parsec_data_lookup_func_t) NULL,
    .flow = &flow_of_dtrsm_LLT_dgemm_for_D,
    .dep_index = 1,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_dtrsm_for_B,
};
...
static const parsec_flow_t flow_of_dtrsm_LLT_dtrsm_for_B = {
    ...
    .flow_index = 0,
    .flow_datatype_mask = 0x1,
    .dep_in = {&flow_of_dtrsm_LLT_dtrsm_for_B_dep1_atline_35,
	       &flow_of_dtrsm_LLT_dtrsm_for_B_dep2_atline_36},
    .dep_out = {&flow_of_dtrsm_LLT_dtrsm_for_B_dep3_atline_37,
		&flow_of_dtrsm_LLT_dtrsm_for_B_dep4_atline_38}
};

static parsec_data_t *flow_of_dtrsm_LLT_dtrsm_for_A_dep1_atline_33_direct_access(const
										 __parsec_dtrsm_LLT_internal_taskpool_t
										 * __parsec_tp,
										 const
										 __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t
										 * assignments)
{
    ...
    (void) n;
    if (__parsec_tp->super.super.context->my_rank ==
	(int32_t) rank_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - k)))
	return data_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - k));
    return NULL;
}

static const parsec_dep_t flow_of_dtrsm_LLT_dtrsm_for_A_dep1_atline_33 = {
    .cond = NULL,		/*  */
    .ctl_gather_nb = NULL,
    .task_class_id = PARSEC_LOCAL_DATA_TASK_CLASS_ID,	/* dtrsm_LLT_descA */
    .direct_data = (parsec_data_lookup_func_t) & flow_of_dtrsm_LLT_dtrsm_for_A_dep1_atline_33_direct_access,
    .dep_index = 0,
    .dep_datatype_index = 0,
    .belongs_to = &flow_of_dtrsm_LLT_dtrsm_for_A,
};
...
static const parsec_flow_t flow_of_dtrsm_LLT_dtrsm_for_A = {
    ...
    .flow_index = 1,
    .flow_datatype_mask = 0x0,
    .dep_in = {&flow_of_dtrsm_LLT_dtrsm_for_A_dep1_atline_33},
    .dep_out = {NULL}
};

static void
iterate_successors_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es, const __parsec_dtrsm_LLT_dtrsm_task_t * this_task,
				      uint32_t action_mask, parsec_ontask_function_t * ontask, void *ontask_arg)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    parsec_task_t nc;		/* generic placeholder for locals */
    parsec_dep_data_description_t data;
    __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t __jdf2c__tmp_locals = *(__parsec_dtrsm_LLT_dtrsm_parsec_assignment_t *) & this_task->locals;	/* copy of this_task locals in R/W mode to manage local definitions */
    int vpid_dst = -1, rank_src = 0, rank_dst = 0;
    const int k = __jdf2c__tmp_locals.k.value;
    (void) k;
    const int n = __jdf2c__tmp_locals.n.value;
    ...
    nc.chore_id = 0;
#if defined(DISTRIBUTED)
    rank_src = rank_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - k));
#endif
    if (action_mask & 0x3) {	/* Flow of data B */
	data.data = this_task->data._f_B.data_out;
	data.arena = __parsec_tp->super.arenas[PARSEC_dtrsm_LLT_VECTOR_ARENA];
	...
	data.displ = 0;
	if (action_mask & 0x1) {
	    __parsec_dtrsm_LLT_write_B0_task_t *ncc = (__parsec_dtrsm_LLT_write_B0_task_t *) & nc;
	    nc.task_class = __parsec_tp->super.super.task_classes_array[dtrsm_LLT_write_B0.task_class_id];
		...
		RELEASE_DEP_OUTPUT(es, "B", this_task, "B", &nc, rank_src, rank_dst, &data);
		if (PARSEC_ITERATE_STOP ==
		    ontask(es, &nc, (const parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dtrsm_for_B_dep3_atline_37,
	...
	if (action_mask & 0x2) {
		...
		if ((dgemm_k >= (0)) && (dgemm_k <= ((descB->mt - 2)))) {
			...
			if ((dgemm_m >= ((ncc->locals.k.value + 1))) && (dgemm_m <= ((descB->mt - 1)))) {
				...
				if (PARSEC_ITERATE_STOP ==
				    ontask(es, &nc, (const parsec_task_t *) this_task,
					   &flow_of_dtrsm_LLT_dtrsm_for_B_dep4_atline_38, &data, rank_src, rank_dst,
					   vpid_dst, ontask_arg))
				    return;
			    }
			}
...
static void
iterate_predecessors_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es,
					const __parsec_dtrsm_LLT_dtrsm_task_t * this_task, uint32_t action_mask,
					parsec_ontask_function_t * ontask, void *ontask_arg)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
	...
	if (action_mask & 0x2) {
		...
		if ((read_B0_k >= (0)) && (read_B0_k <= ((descB->mt - 1)))) {
		    ...
		    if (PARSEC_ITERATE_STOP ==
			ontask(es, &nc, (const parsec_task_t *) this_task,
			       &flow_of_dtrsm_LLT_dtrsm_for_B_dep1_atline_35, &data, rank_src, rank_dst, vpid_dst,
			       ontask_arg))
			return;
		}
	    }
	...
	if (action_mask & 0x4) {
		...
		if ((dgemm_k >= (0)) && (dgemm_k <= ((descB->mt - 2)))) {
			...
			if ((dgemm_n == (0))) {
			    ...
			    if (PARSEC_ITERATE_STOP ==
				ontask(es, &nc, (const parsec_task_t *) this_task,
				       &flow_of_dtrsm_LLT_dtrsm_for_B_dep2_atline_36, &data, rank_src, rank_dst,
				       vpid_dst, ontask_arg))
				return;
			}
		    }
...
static int release_deps_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dtrsm_task_t * this_task,
					   uint32_t action_mask, parsec_remote_deps_t * deps)
{
    PARSEC_PINS(es, RELEASE_DEPS_BEGIN, (parsec_task_t *) this_task); {
	const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	...
	iterate_successors_of_dtrsm_LLT_dtrsm(es, this_task, action_mask, parsec_release_dep_fct, &arg);
...
static int data_lookup_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dtrsm_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    parsec_assignment_t generic_locals[MAX_PARAM_COUNT];	/* generic task locals */
  ...
  /** Lookup the input data, and store them in the context if any */
	...
	if ((0 == k)) {
	    ...
	    ACQUIRE_FLOW(this_task, "B", &dtrsm_LLT_read_B0, "B", target_locals, chunk);
	    this_task->data._f_B.data_out = chunk;
	} else if ((k >= 1)) {
	    __parsec_dtrsm_LLT_dgemm_parsec_assignment_t *target_locals =
		(__parsec_dtrsm_LLT_dgemm_parsec_assignment_t *) & generic_locals;
	    ...
	    ACQUIRE_FLOW(this_task, "B", &dtrsm_LLT_dgemm, "E", target_locals, chunk);
	    this_task->data._f_B.data_out = chunk;
	}
	this_task->data._f_B.data_in = chunk;	/* flow B */
	this_task->data._f_B.data_repo = entry;
	...
	this_task->data._f_B.data_out = parsec_data_get_copy(chunk->original, target_device);
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(chunk->original, (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dtrsm_for_B, 0);
#endif
    }
	...
	entry = NULL;
	chunk = parsec_data_get_copy(data_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - k)), target_device);
	PARSEC_OBJ_RETAIN(chunk);
	this_task->data._f_A.data_out = chunk;
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(data_of_descA(((descB->mt - 1) - k), ((descB->mt - 1) - k)),
				       (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dtrsm_for_A, 1);
#endif
...
#if defined(PARSEC_PROF_GRAPHER) && defined(PARSEC_PROF_TRACE)
	parsec_prof_grapher_data_input(chunk->original, (parsec_task_t *) this_task, &flow_of_dtrsm_LLT_dtrsm_for_A, 0);
#endif
    }
  /** Generate profiling information */
#if defined(PARSEC_PROF_TRACE)
...
static int datatype_lookup_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es,
					      const __parsec_dtrsm_LLT_dtrsm_task_t * this_task, uint32_t * flow_mask,
					      parsec_dep_data_description_t * data)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
...
static int hook_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dtrsm_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    (void) es;
...
#line 41 "dtrsm_LLT.jdf"
	...
	CORE_dtrsm(side, uplo, trans, diag,
		   tempkm, tempnn, lalpha, A /* descA((descB->mt-1)-k,(descB->mt-1)-k) */ , lda,
...
static int complete_hook_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dtrsm_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
#if defined(DISTRIBUTED)
    ...
    parsec_prof_grapher_task((parsec_task_t *) this_task, es->th_id, es->virtual_process->vp_id,
			     __jdf2c_key_fns_dtrsm.key_hash(this_task->task_class->
							    make_key((parsec_taskpool_t *) this_task->taskpool,
								     ((parsec_task_t *) this_task)->locals), NULL));
#endif /* defined(PARSEC_PROF_GRAPHER) */
    release_deps_of_dtrsm_LLT_dtrsm(es, this_task, PARSEC_ACTION_RELEASE_REMOTE_DEPS | PARSEC_ACTION_RELEASE_LOCAL_DEPS | PARSEC_ACTION_RELEASE_LOCAL_REFS | 0x3,	/* mask of all dep_index */
				    NULL);
    return PARSEC_HOOK_RETURN_DONE;
}

static parsec_hook_return_t release_task_of_dtrsm_LLT_dtrsm(parsec_execution_stream_t * es, parsec_task_t * this_task)
{
    const __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(const __parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
...
static char *dtrsm_LLT_dtrsm_LLT_dtrsm_internal_init_deps_key_functions_key_print(char *buffer, size_t buffer_size,
										  parsec_key_t __parsec_key_,
										  void *user_data)
{
    uint64_t __parsec_key = (uint64_t) (uintptr_t) __parsec_key_;
...
static parsec_key_fn_t dtrsm_LLT_dtrsm_LLT_dtrsm_internal_init_deps_key_functions = {
    .key_equal = parsec_hash_table_generic_64bits_key_equal,
    .key_print = dtrsm_LLT_dtrsm_LLT_dtrsm_internal_init_deps_key_functions_key_print,
    .key_hash = parsec_hash_table_generic_64bits_key_hash
};

/* Needs: min-max count-tasks iterate */
static int dtrsm_LLT_dtrsm_internal_init(parsec_execution_stream_t * es, __parsec_dtrsm_LLT_dtrsm_task_t * this_task)
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) this_task->taskpool;
    int32_t nb_tasks = 0, saved_nb_tasks = 0;
    int32_t __k_min = 0x7fffffff, __k_max = 0;
    int32_t __jdf2c_k_min = 0x7fffffff, __jdf2c_k_max = 0;
    __parsec_dtrsm_LLT_dtrsm_parsec_assignment_t assignments;
    int32_t k, n;
    int32_t __jdf2c_k_start, __jdf2c_k_end, __jdf2c_k_inc;
#if defined(PARSEC_PROF_TRACE) && defined(PARSEC_PROF_TRACE_PTG_INTERNAL_INIT)
    PARSEC_PROFILING_TRACE(es->es_profile,
    ...
    PARSEC_AYU_REGISTER_TASK(&dtrsm_LLT_dtrsm);
    ...
    parsec_hash_table_init(__parsec_tp->super.super.dependencies_array[0],
			   offsetof(parsec_hashable_dependency_t, ht_item), 10,
			   dtrsm_LLT_dtrsm_LLT_dtrsm_internal_init_deps_key_functions, this_task->taskpool);
    __parsec_tp->repositories[0] =
	data_repo_create_nothreadsafe(nb_tasks, __jdf2c_key_fns_dtrsm, (parsec_taskpool_t *) __parsec_tp, 2);
    (void) saved_nb_tasks;
    (void) __k_min;
    (void) __k_max;
    (void) __jdf2c_k_start;
...
static const __parsec_chore_t __dtrsm_LLT_dtrsm_chores[] = {
    {.type = PARSEC_DEV_CPU,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) hook_of_dtrsm_LLT_dtrsm},
    {.type = PARSEC_DEV_NONE,
     .evaluate = NULL,
     .hook = (parsec_hook_t *) NULL},	/* End marker */
};
...
static const parsec_task_class_t dtrsm_LLT_dtrsm = {
    ...
    .nb_parameters = 2,
    .nb_locals = 2,
    .params = {&symb_dtrsm_LLT_dtrsm_k, &symb_dtrsm_LLT_dtrsm_n, NULL},
    .locals = {&symb_dtrsm_LLT_dtrsm_k, &symb_dtrsm_LLT_dtrsm_n, NULL},
    .data_affinity = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_dtrsm,
    .initial_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_dtrsm,
    .final_data = (parsec_data_ref_fn_t *) affinity_of_dtrsm_LLT_dtrsm,
    .priority = NULL,
    .properties = properties_of_dtrsm_LLT_dtrsm,
...
#endif /* MAX_PARAM_COUNT */
    .in = {&flow_of_dtrsm_LLT_dtrsm_for_B, &flow_of_dtrsm_LLT_dtrsm_for_A, NULL},
    .out = {&flow_of_dtrsm_LLT_dtrsm_for_B, NULL},
    .flags = 0x0 | PARSEC_HAS_IN_IN_DEPENDENCIES | PARSEC_USE_DEPS_MASK,
    .dependencies_goal = 0x3,
    .make_key = __jdf2c_make_key_dtrsm,
    .key_functions = &__jdf2c_key_fns_dtrsm,
    .fini = (parsec_hook_t *) NULL,
    .incarnations = __dtrsm_LLT_dtrsm_chores,
    .find_deps = parsec_hash_find_deps,
    .iterate_successors = (parsec_traverse_function_t *) iterate_successors_of_dtrsm_LLT_dtrsm,
    .iterate_predecessors = (parsec_traverse_function_t *) iterate_predecessors_of_dtrsm_LLT_dtrsm,
    .release_deps = (parsec_release_deps_t *) release_deps_of_dtrsm_LLT_dtrsm,
    .prepare_input = (parsec_hook_t *) data_lookup_of_dtrsm_LLT_dtrsm,
    .prepare_output = (parsec_hook_t *) NULL,
    .get_datatype = (parsec_datatype_lookup_t *) datatype_lookup_of_dtrsm_LLT_dtrsm,
    .complete_execution = (parsec_hook_t *) complete_hook_of_dtrsm_LLT_dtrsm,
    .release_task = &release_task_of_dtrsm_LLT_dtrsm,
#if defined(PARSEC_SIM)
    .sim_cost_fct = (parsec_sim_cost_fct_t *) NULL,
#endif
};
...
static const parsec_task_class_t *dtrsm_LLT_task_classes[] = {
    &dtrsm_LLT_dtrsm,
    &dtrsm_LLT_read_B0,
    &dtrsm_LLT_write_B0,
    &dtrsm_LLT_dgemm
...
static void dtrsm_LLT_startup(parsec_context_t * context, __parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp,
			      parsec_list_item_t ** ready_tasks)
{
    ...
    for (i = 0; i < PARSEC_dtrsm_LLT_NB_TASK_CLASSES; i++) {
	...
	task->priority = -1;
	task->task_class = task->taskpool->task_classes_array[PARSEC_dtrsm_LLT_NB_TASK_CLASSES + i];
...
static void dtrsm_LLT_destructor(__parsec_dtrsm_LLT_internal_taskpool_t * __parsec_tp)
{
    uint32_t i;
...
parsec_dtrsm_LLT_taskpool_t *parsec_dtrsm_LLT_new(dplasma_enum_t side, dplasma_enum_t uplo, dplasma_enum_t trans,
						  dplasma_enum_t diag, double alpha,
						  const parsec_tiled_matrix_dc_t * descA /* data descA */ ,
						  parsec_tiled_matrix_dc_t * descB /* data descB */ )
{
    __parsec_dtrsm_LLT_internal_taskpool_t *__parsec_tp =
	(__parsec_dtrsm_LLT_internal_taskpool_t *) calloc(1, sizeof(__parsec_dtrsm_LLT_internal_taskpool_t));
    parsec_task_class_t *tc;
    uint32_t i, j;
    /* Dump the hidden parameters with default values */

    __parsec_tp->super.super.nb_task_classes = PARSEC_dtrsm_LLT_NB_TASK_CLASSES;
    __parsec_tp->super.super.devices_index_mask = PARSEC_DEVICES_ALL;
    __parsec_tp->super.super.taskpool_name = strdup("dtrsm_LLT");
	...
	memcpy(tc, dtrsm_LLT_task_classes[i], sizeof(parsec_task_class_t));
	...
	memcpy((__parsec_chore_t *) tc->incarnations, dtrsm_LLT_task_classes[i]->incarnations,
    ...
    tc->prepare_input = (parsec_hook_t *) dtrsm_LLT_dgemm_internal_init;
    /* Startup task for write_B0 */
    tc = (parsec_task_class_t *) __parsec_tp->super.super.task_classes_array[__parsec_tp->super.super.nb_task_classes +
									     1];
    tc->name = "Startup for write_B0";
    tc->prepare_input = (parsec_hook_t *) dtrsm_LLT_write_B0_internal_init;
    /* Startup task for read_B0 */
    tc = (parsec_task_class_t *) __parsec_tp->super.super.task_classes_array[__parsec_tp->super.super.nb_task_classes +
									     2];
    tc->name = "Startup for read_B0";
    tc->prepare_input = (parsec_hook_t *) dtrsm_LLT_read_B0_internal_init;
    ...
    tc->prepare_input = (parsec_hook_t *) dtrsm_LLT_dtrsm_internal_init;
    /* Compute the number of arenas: */
    /*   PARSEC_dtrsm_LLT_FULL_ARENA  ->  0 */
    /*   PARSEC_dtrsm_LLT_VECTOR_ARENA  ->  1 */
    __parsec_tp->super.arenas_size = 2;
...
#if defined(PARSEC_PROF_TRACE)
    __parsec_tp->super.super.profiling_array = dtrsm_LLT_profiling_array;
    if (-1 == dtrsm_LLT_profiling_array[0]) {
#if defined(PARSEC_PROF_TRACE_PTG_INTERNAL_INIT)
	parsec_profiling_add_dictionary_keyword("dgemm (internal init)", "fill:34D8D8",
						...
						(int *) &__parsec_tp->super.super.profiling_array[0 +
												  ...
												  task_class_id +
												  2 *
												  PARSEC_dtrsm_LLT_NB_TASK_CLASSES
    ...
    __parsec_tp->super.super.startup_hook = (parsec_startup_fn_t) dtrsm_LLT_startup;
    __parsec_tp->super.super.destructor = (parsec_destruct_fn_t) dtrsm_LLT_destructor;
...
/* Prevent warnings related to not used hidden global variables */
    ;
    return (parsec_dtrsm_LLT_taskpool_t *) __parsec_tp;
}
